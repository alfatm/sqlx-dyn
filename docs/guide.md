...
Why this crate exists, what it checks, and how it is tested. The marker syntax
and the per-macro reference live in the [API docs](https://docs.rs/sqlx_dyn);
this file covers the parts that do not fit there. `cargo doc --open` renders
them from the source if the crate is not published yet.
...
`AND`/`OR`, a clause keyword, or the `)`/`;` closing the construct around it.
Groups opened in that text are part of the operand, keywords inside them
included.
...
Another `${...}` in that text is a compile error: the predicate straddles it
and cannot be removed as one piece. A clause boundary between the two is what
makes them separable, so `a = ${?x} AND b = ${y}` is fine while
`a = ${?x} || ${y}` is not, and neither is `a = ${?x} + f(${y})`, where the
group is opened by the trailing text itself.

A `#{...}` at the top level of that text is not rejected: a fragment's SQL is
opaque and may _be_ the boundary, which puts it under the fragment constraint
below. Inside such a group it cannot be the boundary, so it is rejected like a
`${...}`.
...
The same opacity carries a second, narrower one:

> A fragment placed at the top level of a predicate's trailing text must **be**
> a clause boundary, not a continuation of the predicate.

`WHERE a = ${?x} #{ORDER_BY_ID}` works. A continuation such as
`SqlFragment::new("|| 'x'")` leaves `SELECT * FROM t || 'x'` once the predicate
drops. Only the top level is unenforced: a fragment nested in a group the
trailing text opened cannot be the boundary, so `a = ${?x} + f(#{F})` is a
compile error.
...
Where a literal _ends_ follows PostgreSQL's default
...
| `code'a\'` | ordinary data — a _type-prefixed_ literal does not escape       |
...
to contain `--`, and it is left alone. `standard_conforming_strings = off` is
not supported.
...
- **A fragment's own leading `WHERE`** — `sql_fragment!("WHERE x = 1")` opens
  the very clause the surrounding predicates already belong to, so bookkeeping
  and SQL agree.
...
text layer _above_ SQL: markers are found by position in the template, before
...
comment covers — decide how a template is _checked_, never whether a marker is a
marker.
...
- `tests/e2e.rs` runs generated queries against a real PostgreSQL 16 in Docker,
  ...
  ```sh
  cargo test                     # 274 tests, no Docker
  cargo test --features e2e      # + 16 tests against a real server
  ```

  The only layer proving the generated SQL is valid PostgreSQL rather than
  merely the text intended. It covers every survival combination of a
  three-filter template, casts and concatenation after a dropped predicate,
  `WHERE`/`HAVING`/`UNION` as independent lists, and injection payloads
  arriving as data. Reverting either optional-predicate fix makes it fail with
  a real server error (`operator does not exist: uuid = text`) — a check no
  text comparison can make.

One limit: that bind values travel out-of-band is a property of sqlx's wire
protocol. These tests verify the values reach sqlx correctly and that the server
treats them as data, not the wire encoding itself.
